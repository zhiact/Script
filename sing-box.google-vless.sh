...
configure_sing_box() {
    ...
    info "正在生成 sing-box 配置文件: ${SB_CONFIG_FILE} (VLESS over WS)"
    local vless_ws_path="/${sb_uuid}-vl" # 为 VLESS 定义一个新的 WebSocket 路径，避免与旧配置冲突

    # 使用 jq 生成 JSON
    # 注意 VLESS 的用户配置和 VMess 不同
    if ! run_sudo sh -c "jq -n \
        ...
        '{
            ...
            \"inbounds\": [
                {
                    \"type\": \"vless\", # <--- 修改点：类型从 vmess 改为 vless
                    \"tag\": \"vless-ws-in\", # 标签也相应修改
                    \"listen\": \"::\",
                    \"listen_port\": \$port,
                    \"users\": [
                        {
                            \"uuid\": \$uuid,
                            \"flow\": \"\" # 基础 VLESS over WS 通常不需要 flow；若要 Vision, 需配合 reality 或 tls
                                        # 例如 "xtls-rprx-vision" (通常与 Reality 或 TLS + Vision 配合)
                                        # 这里保持简单，不启用 Vision 流控，如需启用，客户端和服务端 transport 也需对应配置
                        }
                    ],
                    \"transport\": {
                        \"type\": \"ws\",
                        \"path\": \$path,
                        \"early_data_header_name\": \"Sec-WebSocket-Protocol\" # 可选，用于早期数据混淆
                    }
                    # 如果要启用 TLS (由 sing-box 直接处理，而不是依赖 CF Tunnel 的 TLS):
                    # \"tls\": {
                    #     \"enabled\": true,
                    #     \"server_name\": \"your.domain.com\", # 需要与证书匹配的域名
    ...
    success "sing-box 配置完成 (VLESS over WS)。配置文件位于: ${SB_CONFIG_FILE}"
}
...
generate_output_links() {
    info "正在生成 sing-box (VLESS) 连接信息..."
    local vless_address=""
    local vless_port=""
    local vless_host_header=""       # WebSocket Host 头部
    # 使用 configure_sing_box 中定义的 vless_ws_path
    local vless_path_query="/${sb_uuid}-vl" # <--- 修改点：确保路径与服务端配置一致
    local vless_security="none"      # 默认无 TLS (如果 CF Tunnel 处理 TLS，则客户端看来是 ws)
    local vless_sni=""               # SNI，用于 TLS
    local vless_type="ws"            # 传输类型
    local vless_remark="sing-box-VLESS-ws"
    local flow_control=""            # 默认为空，对应服务端基础配置
                                     # 如果服务端启用了 Vision, 这里可能是 "xtls-rprx-vision"

    # ... (vmess_address, vmess_port, vmess_host_header, vmess_security, vmess_sni 的设置逻辑与之前类似)
    # 我们将这些变量重命名为 vless_...
    if [ "${cf_use_tunnel}" != "no" ]; then
        if [ -n "${cf_domain}" ]; then # 优先使用用户指定或自动检测的 cf_domain
            vless_address="${cf_domain}"
            vless_port="443" # Cloudflare 默认使用 443 HTTPS
            vless_host_header="${cf_domain}"
            vless_security="tls" # <--- 修改点：当通过 CF Tunnel 时，这里应为 tls
            vless_sni="${cf_domain}"
            vless_remark="sing-box_VLESS_CF_${cf_domain}"
        else
            warn "Cloudflare Tunnel 已启用，但未能确定连接域名。"
            warn "生成的 VLESS 链接中的地址可能需要您手动修改。"
            vless_address="YOUR_CLOUDFLARE_DOMAIN"
            vless_port="443"
            vless_security="tls" # <--- 修改点
            vless_sni="YOUR_CLOUDFLARE_DOMAIN"
            vless_remark="sing-box_VLESS_CF_CheckDomain"
        fi
    else # 不使用 Cloudflare Tunnel
        local server_ip
        server_ip=$(curl -s -m 5 ip.sb || curl -s -m 5 ifconfig.me || hostname -I | awk '{print $1}' || echo "YOUR_SERVER_IP")
        vless_address="${server_ip}"
        vless_port="${sb_port}" # 使用 sing-box 的监听端口
        vless_host_header="${server_ip}"
        vless_security="none" # 直连通常不使用 TLS，除非用户在 sing-box 中自行配置了 TLS
        vless_sni=""
        vless_remark="sing-box_VLESS_Direct_${vmess_address}"
        if [[ "${vless_address}" == "YOUR_SERVER_IP" ]]; then
            warn "未启用 Cloudflare Tunnel，且未能自动获取服务器公网IP。"
        else
            warn "未启用 Cloudflare Tunnel。以下链接中的地址 '${vless_address}' 是脚本尝试获取的IP。"
        fi
        warn "您可能需要手动将其修改为服务器的实际公网IP或可访问地址，并确保防火墙允许端口 ${sb_port}。"
        # 如果 sing-box 自己处理 TLS，这里 security 应为 "tls"，sni 应为相应域名，端口可能是 443
    fi

    # VLESS 链接参数构建
    # vless://<uuid>@<address>:<port>?type=<type>&security=<security>&path=<path>&host=<host>&sni=<sni>&flow=<flow>#<remark>
    # 注意：参数需要 URL 编码，特别是 path 和 host 中的特殊字符。
    # 为了简化，这里假设 path 和 host 不包含需要复杂编码的字符。实际应用中可能需要。

    # 对 WebSocket 路径进行 URL 编码 (简单替换 / 为 %2F)
    local encoded_path
    encoded_path=$(echo -n "${vless_path_query}" | jq -sRr @uri) # jq 的 @uri 可以做 URL 编码

    local vless_link_params="type=${vless_type}"
    vless_link_params+="&security=${vless_security}"
    vless_link_params+="&path=${encoded_path}" # 使用编码后的路径

    if [ -n "${vless_host_header}" ]; then
        vless_link_params+="&host=${vless_host_header}" # host 通常是 WebSocket 的 Host 头部
    fi
    if [ -n "${vless_sni}" ]; then
        vless_link_params+="&sni=${vless_sni}" # SNI 用于 TLS 握手
    fi
    if [ -n "${flow_control}" ]; then
        vless_link_params+="&flow=${flow_control}"
    fi
    # 也可以添加 &fp=<fingerprint> (TLS指纹) 等参数，这里保持简单

    local vless_link="vless://${sb_uuid}@${vless_address}:${vless_port}?${vless_link_params}#$(echo -n "${vless_remark}" | jq -sRr @uri)"

    echo -e "\n${GREEN}================ Sing-box (VLESS) 安装完成 ================${PLAIN}"
    echo -e "  Sing-box UUID:     ${YELLOW}${sb_uuid}${PLAIN}"
    echo -e "  Sing-box 端口:    ${YELLOW}${sb_port}${PLAIN}"
    echo -e "  WebSocket 路径:  ${YELLOW}${vless_path_query}${PLAIN}" # 显示原始路径
    if [ "${cf_use_tunnel}" != "no" ]; then
    ...
    echo -e "\n${GREEN}VLESS 连接链接:${PLAIN}"
    echo -e "${YELLOW}${vless_link}${PLAIN}\n"

    # 生成二维码 (如果 qrencode 已安装)
    if command -v qrencode &>/dev/null; then
        echo -e "${GREEN}VLESS 二维码 (部分终端可能无法完美显示):${PLAIN}"
